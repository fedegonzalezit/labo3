...
class CustomTensorDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            y (ndarray or Tensor): Target variable.
            weights (ndarray or Tensor): Weights.
            serie_id (ndarray or Tensor): Serue)id IDs.
            dates (ndarray or Tensor): Date IDs.
            times (ndarray or Tensor): Time IDs.
            on_batch (bool): If True, data is reshaped when calling `__getitem__`. 
                Required if the number of time IDs per date is not uniform.
...
class SeriesMonthlyDataset(Dataset):
    """
    Dataset para series temporales mensuales.
    Cada muestra es una secuencia de 12 meses para una serie (serieID).
    """
    def __init__(self, df, feature_cols, target_col, serie_id_col, fecha_col, window=12):
        self.window = window
        self.feature_cols = feature_cols
        self.target_col = target_col
        self.serie_id_col = serie_id_col
        self.fecha_col = fecha_col

        # Ordenar por serie y fecha
        df = df.sort_values([serie_id_col, fecha_col])
        self.groups = df.groupby(serie_id_col)

        self.samples = []
        for serie_id, group in self.groups:
            if len(group) < window:
                continue
            for i in range(len(group) - window + 1):
                window_df = group.iloc[i:i+window]
                X = window_df[feature_cols].values.astype(np.float32)
                y = window_df[target_col].values.astype(np.float32)
                # Reemplaza NaN por 0 en features y target
                X = np.nan_to_num(X, nan=0.0)
                y = np.nan_to_num(y, nan=0.0)
                self.samples.append((X, y, serie_id))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        X, y, serie_id = self.samples[idx]
        return torch.tensor(X), torch.tensor(y), serie_id

class SimpleRNNModel(nn.Module):
    def __init__(self, input_size, hidden_size=64, rnn_type="gru"):
        super().__init__()
        if rnn_type == "gru":
...
class NNPipelineModel:
    ...
    def fit(self, df, feature_cols, target_col, serie_id_col, fecha_col):
        # df: DataFrame con todas las columnas
        # feature_cols: lista de features
        # target_col: nombre del target
        # serie_id_col: nombre del ID de serie
        # fecha_col: nombre de la fecha
        ...
        self.feature_cols = feature_cols
        dataset = SeriesMonthlyDataset(df, feature_cols, target_col, serie_id_col, fecha_col, window=self.params["window"])
        loader = torch.utils.data.DataLoader(dataset, batch_size=self.params["batch_size"], shuffle=True)

        self.model = SimpleRNNModel(len(feature_cols), self.params["hidden_size"], self.params["rnn_type"]).to(self.params["device"])
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.params["lr"])
        loss_fn = nn.MSELoss()

        print(f"[NNPipelineModel] Training model with {len(dataset)} samples, ")
        self.model.train()
        for epoch in range(self.params["epochs"]):
            ...
            print(f"[NNPipelineModel] Epoch {epoch+1}/{self.params['epochs']} - Loss: {avg_loss:.4f}")

    def predict(self, X):
        # X debe ser un DataFrame con todas las columnas
        X = X.copy()
        ...
        X = X.sort_values(['serieID', 'fecha'])
        idx_map = dict(zip(X.index, range(len(X))))
        window = self.params["window"]

        with torch.no_grad():
            for serie_id, group in X.groupby('serieID'):
                group = group.sort_values('fecha')
                group_features = group[self.feature_cols].values.astype(np.float32)
                group_idx = group.index.tolist()
                for i in range(len(group)):
                    # Tomar los últimos `window` pasos (completar con 0 si faltan)
                    start = max(0, i - window + 1)
                    window_X = group_features[start:i+1]
                    if window_X.shape[0] < window:
                        pad = np.zeros((window - window_X.shape[0], window_X.shape[1]), dtype=np.float32)
                        window_X = np.vstack([pad, window_X])
                    window_X_tensor = torch.tensor(window_X).unsqueeze(0).to(self.params["device"])
                    y_pred = self.model(window_X_tensor).cpu().numpy().flatten()[-1]
                    preds[idx_map[group_idx[i]]] = y_pred
        return preds
    

from pipeline import PipelineStep
class TrainNNModelStep(PipelineStep):
    def __init__(self, model_cls=NNPipelineModel, name=None, params=None):
        super().__init__(name)
        self.model_cls = model_cls
        self.params = params or {}

    def execute(self, df, train_index, features):
        # df debe tener todas las columnas: serieID, fecha, features, target
        # df_val es el set de validación (opcional)
        model = self.model_cls(self.params)
        train = df.loc[train_index]
        model.fit(train, features, 'target', 'serieID', 'fecha')
        return {"model": model}
    
import pandas as pd
class PredictNNModelStep(PipelineStep):
    def execute(self, model, df, test_index, features):
        # df debe tener todas las columnas: serieID, fecha, features
        test = df.loc[test_index]
        predictions = model.predict(test)
        print(predictions)
        predictions = pd.Series(predictions, index=test_index, name='predictions')
        return {
